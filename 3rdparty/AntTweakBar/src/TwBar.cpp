        ...
        */
        ...
        else
        {
            ...
            if( Atom!=NULL && Atom->m_Visible )
            {
                ...
                CTwVarAtom *Var = static_cast<CTwVarAtom *>(m_HierTags[m_HighlightedLine].m_Var);
                if( (Var->m_Type==TW_TYPE_CDSTRING || Var->m_Type==TW_TYPE_CDSTDSTRING || Var->m_Type==TW_TYPE_STDSTRING || IsCSStringType(Var->m_Type))
                    && EditInPlaceAcceptVar(Var) && !m_EditInPlace.m_Active )
                {
                    EditInPlaceStart(Var, m_VarX1, m_VarY0+(m_HighlightedLine)*(m_Font->m_CharHeight+m_LineSep), m_VarX2-m_VarX1-1);
                }
